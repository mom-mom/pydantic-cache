...
class TestNullableTypes:
    ...
    async def test_none_value_caching(self, cache_setup):
        ...
        async def get_optional_value(key: str) -> Optional[str]:
            nonlocal call_count
            call_count += 1
            if key == "none":
                return None
    ...
    async def test_optional_pydantic_model(self, cache_setup):
        ...
        async def get_user_profile(user_id: int) -> Optional[UserProfile]:
            ...
            return UserProfile(
                id=user_id,
                name=f"User {user_id}",
                bio=None if user_id == 1 else f"Bio for user {user_id}",
                age=None
            )
    ...
    async def test_union_types(self, cache_setup):
        ...
        async def get_mixed_type(key: str) -> Union[str, int, None]:
            nonlocal call_count
            call_count += 1
            if key == "string":
                return "text"
    ...
    async def test_nested_optional(self, cache_setup):
        ...
        async def get_nested_data(key: str) -> Optional[List[Optional[str]]]:
            nonlocal call_count
            call_count += 1
            if key == "none":
                return None
    ...
    async def test_none_with_different_coders(self):
        ...
        for coder_class in [JsonCoder, PickleCoder]:
            ...
            async def get_optional(key: str) -> Optional[dict]:
                nonlocal call_count
                call_count += 1
                if key == "none":
                    return None
    ...
    async def test_concurrent_none_caching(self, cache_setup):
        ...
        async def maybe_slow_function(value: int) -> Optional[int]:
            nonlocal call_count
            call_count += 1
